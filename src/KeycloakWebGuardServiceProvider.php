...
namespace Ebrook\KeycloakWebGuard;
...
use Ebrook\KeycloakWebGuard\Auth\Guard\KeycloakWebGuard;
use Ebrook\KeycloakWebGuard\Auth\KeycloakWebUserProvider;
use Ebrook\KeycloakWebGuard\Middleware\KeycloakAuthenticated;
use Ebrook\KeycloakWebGuard\Middleware\KeycloakCan;
use Ebrook\KeycloakWebGuard\Middleware\KeycloakCanOne;
use Ebrook\KeycloakWebGuard\Services\KeycloakService;
...
class KeycloakWebGuardServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        Auth::extend('keycloak-web', function ($app, $name, array $config) {
            ...
            return new KeycloakWebGuard($provider, $app->request, $name);
        });

        // Facades
        $this->app->singleton('keycloak-web', function($app) {
    ...
    private function registerRoutes()
    {
        ...
        if (! empty($routes['login'])) {
            $router->middleware('web')->get($routes['login'], 'Ebrook\KeycloakWebGuard\Controllers\AuthController@login')->name('keycloak.login');
        }

        if (! empty($routes['logout'])) {
            $router->middleware('web')->get($routes['logout'], 'Ebrook\KeycloakWebGuard\Controllers\AuthController@logout')->name('keycloak.logout');
        }

        if (! empty($routes['register'])) {
            $router->middleware('web')->get($routes['register'], 'Ebrook\KeycloakWebGuard\Controllers\AuthController@register')->name('keycloak.register');
        }

        if (! empty($routes['callback'])) {
            $router->middleware('web')->get($routes['callback'], 'Ebrook\KeycloakWebGuard\Controllers\AuthController@callback')->name('keycloak.callback');
        }
    }
}
